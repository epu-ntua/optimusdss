...
if ((data$pv3[i]+data$pv2[i]+data$pv1[i])<0){
data$pv1[i]=data$pv2[i]=data$pv3[i]<-0
}
}

data$Load<-data$canteen1+data$canteen2+data$canteen3+data$main1+data$main2+data$main3+data$pv1+data$pv2+data$pv3

data$canteen1=data$canteen2=data$canteen3=data$main1=data$main2=data$main3=data$pv1=data$pv2=data$pv3<-NULL
for (i in 1:nrow(data)){
if (data$Load[i]<0){
data$Load[i]<-0
}
...
######################################         Dataset per hour                   ####################################
tempweather <- weather[,!names(weather) %in% c("Year","ID","Oldness","irradiation",
...
PredictionEnergy <- function(tempweather,dataDatetime){
  ...
  for (i in 1:length(Hours)){
    model<-regsubsets(Load ~ .  ,intercept=TRUE ,data=Hours[[i]])
    summodel<-summary(model)
    Totalmodels[length(Totalmodels)+1]<-list(regsubsets(Load ~ . ,intercept=TRUE ,data=Hours[[i]],nvmax=which.min(summodel$bic)))
  }
  #Estimate forecasts
  predictions<-NULL
  Hours<-NULL
  ...
  final.datatable <- final.datatable[,!names(final.datatable) %in% c("air_temperature","irradiation","relative_humidity","pressure",
