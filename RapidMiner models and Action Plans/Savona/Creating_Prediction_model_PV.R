...
if ((data$Phase3[i]+data$Phase2[i]+data$Phase1[i])<0){
data$Phase1[i]=data$Phase2[i]=data$Phase3[i]<-0
}
}

data$pv_power<-(data$Phase1+data$Phase2+data$Phase3)
data$Phase1=data$Phase2=data$Phase3<-NULL

MaxValues<-c()
for (i in 0:23){
MaxValues<-c(MaxValues,as.numeric(max(data[data$Hour==i,]$pv_power)))
}

list25<-c()
for (h in 0:23){
check1<-data[data$Hour==h,]$pv_power
check2<-data[data$Hour==h,]$irradiation_forecast
...
######################################         Dataset per hour                   ####################################
tempweather <- weather[,!names(weather) %in% c("Year","ID","Oldness","Month","relative_humidity_forecast")]
...
predictPV<-function(tempweather){
  ...
  for (h in from:to){
    ...
    #}
    Hours[[length(Hours)]]<- Hours[[length(Hours)]][Hours[[length(Hours)]]$pv_power>0,]
  }
  ...
  final.datatable <- final.datatable[,!names(final.datatable) %in% c("air_temperature_forecast",
...
Totalmodels<-list(prmdls[[2]],c(from,to),c(MaxValues))