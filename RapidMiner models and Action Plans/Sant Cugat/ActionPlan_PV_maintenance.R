...
trainmodel<-function(tempweather){
  ...
  for (h in from:to){
    ...
    Hours[[length(Hours)]]<- Hours[[length(Hours)]][Hours[[length(Hours)]]$pv_power>10,]
  }
  # For the rest is equal to zero.....
  ####################################################################################################################################################
  #Calculate models
  TotalHours<-Hours
  n<-lm(pv_power ~ .,data=TotalHours[[1]])
  Totalmodels<-NULL
  includeInt = FALSE
  for (i in 1:11){
    model<-regsubsets(pv_power ~ .  ,intercept=includeInt ,data=TotalHours[[i]])
    summodel<-summary(model)
    Totalmodels[length(Totalmodels)+1]<-list(regsubsets(pv_power ~ . ,intercept=includeInt ,data=TotalHours[[i]],nvmax=which.min(summodel$bic)))
  }
  #Estimate forecasts
  predictions<-NULL
  Hours<-NULL
  ...
  final.datatable <- final.datatable[,!names(final.datatable) %in% c("irradiation",
...
maintenance<-function(final.datatable){
  ...
  Threshold=as.numeric(boxplot(Risk$TotalRisk,plot=FALSE)$stats[5])
  Risk$Alert=0
...
final.datatable<-maintenanceout[[3]]
